...
from citagro.models import Administrador,Inputoperacion,Cultivoexplotacion,Tiposoperacion,Cultivo,Tecnico,Perfil,Explotacion,UsuarioExplotacion,MaquinaExplotacion,DatosClimaticos,Provincia,Municipio,Maquinaria,User,Aplicador,UHC,Parcela,Analisis,Aplicacion,Producto_aplicado,Operacion,op_poda,op_labores,op_fertilizacion,op_riego,op_recoleccion
...
class InputoperacionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id','input','idTipoOp','coefEmision','cantidad','coefAsignacion','costeUnitario','unidad','unidadCoef','tipoOperacion','orden','idTipoOperacion')
...
class ParcelaSerializer(serializers.ModelSerializer):		
    ...
    nombreCultivo = serializers.ReadOnlyField(source='idcultivo.idCultivo.nombre')

    class Meta:
        model = Parcela
        fields = ('id','nombre','provincia','parcela_codProv','parcela_Prov','municipio','parcela_codMun','parcela_Mun','poligono','parcela','recinto','superficie_hectareas','sr','descripcion','idUHC','uhc_nombre','idExplotacion','idcultivo','nombreCultivo')
...
class muestra_fenologiaSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'idMuestra','muestra_a','muestra_b','muestra_c','muestra_d','muestra_e','muestra_f','muestra_a1','muestra_a2','muestra_a3','muestra_e1','muestra_e2','muestra_e3','muestra_f1')

	
class muestra_insetosSerializer(serializers.ModelSerializer):
    class Meta:
        model = muestra_insetos
        fields = ('id', 'idMuestra','coccinelidos','neuropteros','sirfidos','fitoseidos','scutellista','apanteles','aphytis')
		
class muestra_trampasSerializer(serializers.ModelSerializer):
    class Meta:
        model = muestra_trampas
        fields = ('id', 'idMuestra','carpocapsa')
...
class muestra_enfermedadesSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'idMuestra','bacteriosis','antracnosis')
...
class ExplotacionSerializer(serializers.ModelSerializer):
    ...
    parcelasExplotacion = ParcelaSerializer(many=True,read_only=True)
