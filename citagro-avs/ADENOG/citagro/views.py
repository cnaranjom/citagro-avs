...
import unicodedata
import win32api
from citagro.models import Document,Inputoperacion,Cultivoexplotacion,Tiposoperacion,Administrador,Cultivo,Tecnico,Perfil,Explotacion,UsuarioExplotacion,DatosClimaticos,Provincia,Municipio,MaquinaExplotacion,Maquinaria,Aplicador,UHC,Analisis,Parcela,Aplicacion,Producto_aplicado,MaquinaExplotacion,Operacion,op_poda,op_labores,op_fertilizacion,op_riego,op_recoleccion
...
from forms import FormularioTecnico,FormularioAdministrador,FormularioExplotacion,AsignaUsuarioFormulario,FormularioMaquina,FormularioClima,UploadForm
...
from citagro.serializers import PerfilSerializer,InputoperacionSerializer,TiposoperacionSerializer,CultivoexplotacionSerializer,AdministradorSerializer, CultivoSerializer,TecnicoSerializer,ExplotacionSerializer,DatosClimaticosSerializer,MaquinariaSerializer,UsuariosSerializer,ProvinciaSerializer,MunicipioSerializer,AplicadorSerializer,UHCSerializer,ParcelaSerializer,AnalisisSerializer,AplicacionSerializer,ProductoSerializer,UsuarioExplotacionSerializer,MaquinaExplotacionSerializer,OperacionSerializer,op_podaSerializer,op_laboresSerializer,op_fertilizacionSerializer,op_riegoSerializer,op_recoleccionSerializer
...
def crearParcela(request,string):
	...
	parcela = Parcela.objects.create( 
		...
		sr = sr,
		descripcion = desc,
		idUHC = UHC.objects.get(id=idUhc),
		idExplotacion = Explotacion.objects.get(id=idExpl),
		idcultivo = Cultivoexplotacion.objects.get(id=idcult)
	)
	return HttpResponseRedirect('/tecnico/explotacion/'+idExpl+'/'+idUhc)
    ...
    if request.user.is_authenticated(): 
        if not request.user.is_staff:
            ...
            if tipoId == 'T':
				...
				parcelas = Parcela.objects.filter(idUHC__in=listauhc)
...
def creaExplotacion(request):
    ...
    if request.method == 'POST':
        ...
        explotacion = Explotacion(Razon_Social=request.POST['Razon_Social'],cif_nif = request.POST['cif_nif'],Domicilio = request.POST['Domicilio'],Localidad = Muni,Provincia = Provi,CP = request.POST['CP'],idAdmin = admin)
    ...
    if request.user.is_authenticated(): 
        if not request.user.is_staff:
            ...
            if tipoId == 'T':
				...
				documento = "";
				uhcParcelas = Parcela.objects.filter(idUHC = analisis.idUHC)
    ...
    if request.user.is_authenticated(): 
        if not request.user.is_staff:
            ...
            if tipoId == 'T':
				...
				uhcParcelas = Parcela.objects.filter(idUHC = aplicacion.idUHC)
    ...
    if request.user.is_authenticated(): 
        if not request.user.is_staff:
            ...
            if tipoId == 'T':
				...
				recoleccion = ''
				tipoP = ''
				uhcParcelas = Parcela.objects.filter(idUHC = operacion.idUHC)
    ...
    if request.user.is_authenticated(): 
        if not request.user.is_staff:
            ...
            if tipoId == 'T':
				...
				uhcParcelas = Parcela.objects.filter(idUHC = muestra.idUHC)
...
def VistaConfiguracion(request):
    ...
    Inputoperaciones = Inputoperacion.objects.all().order_by('idTipoOp')
    perfilUsuario = Perfil.objects.get(user_id = request.user.id)
    tipoId = perfilUsuario.TipoUsuario			
    if tipoId == 'A':
        return HttpResponseRedirect('/administrador/')
    ...
    if request.user.is_authenticated(): 
        if not request.user.is_staff:
            ...
            if tipoId == 'A':
				...
				parcelas = Parcela.objects.filter(idUHC = uhc.id)
...
def generaInformePDF(request,id_Uhc):
	...
	p.drawCentredString(4.3*inch, 770,'Informe')
